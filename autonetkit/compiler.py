...
# and allocate tap interface
    ...
    def dns(self,node):
        """Build dns in the nidb node"""
        g_phy = self.anm['phy']
        phy_node = self.anm['phy'].node(node)
        g_dns = self.anm['dns']
        g_ipv4 = self.anm['ipv4']
        
        if node.is_nameServer or node.is_DNSResolver:
            # cherche l'adresse et le nom du root server
            for n in g_dns.nodes(): 
                ...
                try :   # le noeud cd_12_6 peut causer de problemes
                    ...
                    print("except compiler line 698: "+str(error))
                except Exception, error:
                    print("except compiler line 700: "+str(error))
                    
            # construit la liste des hotes de sa zone
        ...
        if node.is_client:
            for n in g_dns.nodes():
                phy_node = g_phy.node(n)
                try :   # le noeud cd_12_6 peut causer de problemes
                    if phy_node.is_DNSResolver and node.get("DNSResolver") == phy_node.get("label"):
                        node.DNSResolverIpv4 = self.nidb.node(phy_node).interfaces[0].ipv4_address
                        break
                except AttributeError, error:
                    print("except compiler line 740: "+str(error))
                except Exception, error:
                    print("except compiler line 742: "+str(error))                    
             
        #si c'est le root server, on construit lui la liste de tous les domaines presents dans le reseau
            ...
            for n in g_dns.nodes():
                ...
                try :   # le noeud cd_12_6 peut causer de problemes
                    ...
                    if phy_node.is_nameServer and not phy_node.is_rootServer and not n.level != "slave":
                        name = n.domain
                        nameServerName = n.name
                        nameServerIPv4_address = self.nidb.node(phy_node).interfaces[0].ipv4_address
                        domain = Domain(name, nameServerName, nameServerIPv4_address)
                        node.domains.append(domain)
                        break
                except AttributeError, error:
...
class Domain(object):
    ...
    def __init__(self, name, nameServerName, nameServerIPv4_address):
        self.name = name
        self.nameServerName = nameServerName #
        self.nameServerIPv4_address = nameServerIPv4_address # 
        self.clients = []
    
    def addHost(self, hostName, hostAddress, ttl):
        """ la liste des hotes du de la zone """
